...
z2 = X*Theta1';
a2 = sigmoid(z2); % size 25x5000

% Add ones to the a2 hidden matrix
a2 = [ones(size(a2,1), 1) a2]; %size 5000*26
...
% Create a y1 matrix instead of having the values 1-9
% instead have a matrix of 1s and 0s
% size m*K
y1 = zeros(m, K);
for k = 1:K
    y1(:,k) = y == k;
...
% Then do element wise multiplying and sum over m and K 
...
% Recreate the Theta matrices but with a j=1 column of zeros
Theta1_reg = Theta1;
...
% Add them the the unregularized gradients
